...
async function saveItems(items) {
  ...
  resultArr = Array.from(items);
  localList = Array.from(items);
    ...
    if (!editingMode) {
      ...
      localList.forEach((item) => {
        ...
        category.onchange = () => {
          ...
          localList = Array.from(newArr);
          generateList();
          let itemInDOM = elA(".editing-item")[findIndex(item)];
          window.scrollTo({
            top: itemInDOM.getBoundingClientRect().top + window.scrollY - 300,
            behavior: "smooth",
          });
          itemInDOM.style.outline = "5px solid green";
          setTimeout(() => {
            itemInDOM.style.outline = "none";
          }, 2000);
        };
        const x = make("p");
        x.innerText = "X";
...
function test() {
  ...
  testArr.sort((a, b) => {
    const categorySort =
      codeCategories.indexOf(a.category) - codeCategories.indexOf(b.category);

    return categorySort;
  });
  console.log(testArr.find((i) => i.category == "Regulars").name);
  console.log(
    testArr[testArr.findIndex((i) => i.category == "Ingredients") - 1].name,
  );
}
